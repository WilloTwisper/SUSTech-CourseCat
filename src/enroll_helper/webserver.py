    ...
    def _job(self, opts: dict) -> None:
        try:
            ...
            if imminent_start(at_text):
                self.emit("log", text="[i] 临近开抢时刻，跳过开跑检查")
            else:
                self.emit("log", text="[*] 开跑检查（已选剔除 + 余量刷新 + 存活检查）…")
                queue = startup_check(
                    tis, sem, queue, Pacer(self.discovery_ms),
                    log=lambda m: self.emit("log", text=m))
                if not queue:
                    raise RuntimeError("队列已空（课程或已选、或已不在目录），请重新选课")
